...
class RelationshipCog(commands.Cog):
    def __init__(self, bot):
        ...
        self.relationship_descriptions = {
            1: "🔴 Вражда", 2: "🔴 Конфликт", 3: "🟡 Напряжение", 
            4: "🟡 Нейтрально", 5: "🟢 Дружелюбие", 6: "🟢 Симпатия",
            7: "🔵 Дружба", 8: "🔵 Близость", 9: "💖 Любовь", 10: "💖 Душа"
        }
    ...
    async def show_detailed_relationships(self, ctx, *, character_name: str = None):
        ...
        if character_name:
            ...
            if not relationships:
                embed.description = f"У {character_name} пока нет определенных отношений."
            else:
                desc = ""
                for other_char, rel_data in sorted(relationships, key=lambda x: x[1]['value'], reverse=True):
                    desc += f"**{other_char}**: {rel_data['value']} - {rel_data['description']}\n"
                embed.description = f"Отношения **{character_name}**:\n\n{desc}"
        else:
            # Показать все отношения
            desc = ""
            for rel_key, rel_data in sorted(self.system.relationships.items(), 
